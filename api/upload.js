export default async function handler(req, res) {
    ...
    // --- FITUR 1: FITUR CEK IP VERCEL (Dipanggil via GET) ---
    if (req.method === 'GET') {
        try {
            const response = await fetch('https://api.ipify.org?format=json');
            const data = await response.json();
            return res.status(200).json({ ip_vercel: data.ip });
        } catch (error) {
            return res.status(500).json({ error: error.message });
        }
    }

    // --- FITUR 2: FITUR UPLOAD KE GITHUB (Dipanggil via POST) ---
    ...
    try {
        ...
        const token = process.env.REGAL_TOKEN; // Pastikan ini di-set di Vercel
        ...
        return res.status(200).json({ 
            success: true, 
            download_url: `https://regal-yuki.vercel.app/${fileName}` 
        });
    } catch (error) {
        return res.status(500).json({ error: error.message });
    }
