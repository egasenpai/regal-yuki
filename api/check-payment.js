...
export default async function handler(req, res) {
  ...
  try {
    console.log(`[Check] Looking up: ${ref}`);
    const txn = await findTransaction(ref);
    
    if (!txn) {
      console.log(`[Check] Not found: ${ref}`);
      return res.status(404).json({ error: "Transaction not found", ref });
    }

    console.log(`[Check] Found: ${txn.referenceId}, status=${txn.status}, austinTxId=${txn.austinTxId || 'none'}`);

    let austinStatus = null;
    let serverInfo = txn.serverInfo || null;
    let currentStatus = txn.status;

    // Cek ke Austin kalau masih pending/processing
      ...
      try {
        ...
        if (checkRes.ok) {
          ...
          if (austinStatus && paidStatuses.includes(austinStatus.toString().toLowerCase()) && txn.status !== "completed") {
            console.log(`[Check] Austin paid! Running fallback...`);
            ...
            if (freshTxn && freshTxn.status !== "completed") {
              ...
              try {
                ...
                console.log(`[Check] Server created: ${newServerInfo.serverId}`);
              } catch (ptErr) {
                console.error("[Check] Create server failed:", ptErr.message);
        ...
        console.error("[Check] Austin check error:", e.message);
      }
    }
